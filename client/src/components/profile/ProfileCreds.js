  ...
  render() {
    ...
    const eduItems = education.map(edu => (
      <li key={edu._id} className="list-group-item">
        ...
        <p>
          <strong>Diploma:</strong> {edu.degree}
        </p>
        <p>
    ...
    return (
      <div className="row">
        <div className="col-md-6">
          <h3 className="text-center text-info">Experiência profissional</h3>
          {expItems.length > 0 ? (
            <ul className="list-group">{expItems}</ul>
          ) : (
            <p className="text-center">Nemhuma experiência listada</p>
          )}
        </div>

        <div className="col-md-6">
          <h3 className="text-center text-info">Educação</h3>
          {eduItems.length > 0 ? (
